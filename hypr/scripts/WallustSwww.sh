...
# Inputs and paths
passed_path="${1:-}"
cache_dir="$(ls -d "$HOME/.cache/awww"/*/ 2>/dev/null | sort -V | tail -1)"
rofi_link="$HOME/.config/rofi/.current_wallpaper"
wallpaper_current="$HOME/.config/hypr/wallpaper_effects/.wallpaper_current"
read_cached_wallpaper() {
  local cache_file="$1"
  if [[ -f "$cache_file" ]]; then
    # awww cache is NUL-separated; the last field is the wallpaper path
    tr '\0' '\n' < "$cache_file" | awk 'NF {last=$0} END {print last}'
  fi
}
