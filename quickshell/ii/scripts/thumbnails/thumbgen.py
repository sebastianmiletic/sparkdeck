...
def make_thumbnail(fpath: str) -> bool:
    # A single malformed or unusually encoded wallpaper must not abort the
    # entire preload batch. Some GnomeDesktop thumbnailers raise GLib errors
    # instead of returning None, so contain every file independently.
    try:
        ...
        logger.warning("SKIPPED     {} ({})".format(fpath, error))
        return False
