...
Rectangle {
    ...
    readonly property bool fromAssistant: root.messageData?.role === "assistant"

    property list<var> messageBlocks: StringUtils.splitMarkdownBlocks(root.messageData?.content ?? "")

    anchors.left: parent?.left
    anchors.right: parent?.right
    anchors.leftMargin: root.fromUser ? 44 : 4
    anchors.rightMargin: root.fromAssistant ? 4 : 4
    implicitHeight: columnLayout.implicitHeight + root.messagePadding * 2

    radius: Appearance.rounding.normal
    color: root.fromUser ? Appearance.colors.colSecondaryContainer : "transparent"
    border.width: root.fromAssistant ? 0 : 1
    border.color: root.fromUser ? Appearance.colors.colSecondaryContainerHover : Appearance.colors.colLayer0Border

    Behavior on color {
        animation: Appearance.animation.elementMoveFast.colorAnimation.createObject(this)
    }

    function prepareFollowUp(prompt) {
        if (!root.messageInputField) return;
        root.messageInputField.text = prompt;
        root.messageInputField.cursorPosition = prompt.length;
        root.messageInputField.forceActiveFocus();
    }
