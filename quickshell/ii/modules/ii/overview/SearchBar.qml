...
RowLayout {
    ...
    ToolbarTextField { // Search box
        ...
        placeholderText: LauncherSearch.aiConversationActive
            ? (LauncherSearch.aiReadyForFollowUp ? Translation.tr("Ask a follow-up…") : Translation.tr("Answering…"))
            : Translation.tr("Search, calculate or ask anything")
        // A fixed surface width prevents Spotlight from shifting when results
        // appear; the results panel only grows downward.
        implicitWidth: 560
        ...
        onAccepted: {
            const submittedText = searchInput.text.trim();
            if (submittedText.length > 0 && (LauncherSearch.aiConversationActive || LauncherSearch.isLikelyQuestion(submittedText))) {
                LauncherSearch.askAi(submittedText);
                searchInput.text = "";
                searchInput.forceActiveFocus();
                return;
            }
