...
Item { // Wrapper
    ...
    property bool showResults: searchingText != "" || LauncherSearch.aiConversationActive
    ...
    Rectangle { // Background
        ...
        ColumnLayout {
            ...
            ListView { // App results
                id: appResults
                visible: root.searchingText != "" && !LauncherSearch.aiConversationActive
                Layout.fillWidth: true
            ...
            Item {
                id: inlineAiView
                visible: LauncherSearch.aiConversationActive
                Layout.fillWidth: true
                implicitHeight: visible ? Math.min(480, Math.max(190, aiConversationColumn.implicitHeight + 34)) : 0

                Flickable {
                    id: aiConversationFlick
                    anchors.fill: parent
                    anchors.margins: 17
                    clip: true
                    contentWidth: width
                    contentHeight: aiConversationColumn.implicitHeight
                    boundsBehavior: Flickable.StopAtBounds

                    ColumnLayout {
                        id: aiConversationColumn
                        width: aiConversationFlick.width
                        spacing: 12

                        StyledText {
                            Layout.fillWidth: true
                            text: LauncherSearch.aiLastQuestion
                        ...
                        MaterialLoadingIndicator {
                            visible: !(LauncherSearch.aiResponseMessage?.done ?? false)
                                && !(LauncherSearch.aiResponseMessage?.content?.length > 0)
                            loading: visible
                        ...
                        StyledText {
                            ...
                            text: LauncherSearch.aiResponseMessage?.content ?? ""
                        ...
                        RowLayout {
                            ...
                            RippleButton {
                                visible: (LauncherSearch.aiResponseMessage?.content?.length ?? 0) > 0
                                implicitWidth: copyInlineRow.implicitWidth + 20
                                implicitHeight: 32
                                buttonRadius: 16
                                colBackground: Appearance.colors.colLayer1
                                onClicked: Quickshell.clipboardText = LauncherSearch.aiResponseMessage?.content ?? ""

                                contentItem: RowLayout {
                                    id: copyInlineRow
                                    spacing: 5
                                    MaterialSymbol { text: "content_copy"; iconSize: 16; color: Appearance.colors.colOnLayer1 }
                                    StyledText { text: Translation.tr("Copy"); color: Appearance.colors.colOnLayer1; font.pixelSize: Appearance.font.pixelSize.smaller }
                                }
                            }
                            ...
                            StyledText {
                                ...
                                text: LauncherSearch.aiReadyForFollowUp
                                    ? Translation.tr("Type another question for a follow-up")
                                    : Translation.tr("Generating response…")
                                color: Appearance.colors.colSubtext
