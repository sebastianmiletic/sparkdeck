...
Singleton {
    ...
    function cancelWifiConnection(network: WifiAccessPoint): void {
    ...
    function changePassword(network: WifiAccessPoint, password: string, username = ""): void {
        ...
        changePasswordProc.exec({
            ...
            "command": network.isEnterprise
                ? ["bash", "-c", `
                    if ! nmcli connection show "$SSID" >/dev/null 2>&1; then
                        nmcli connection add type wifi ifname "*" con-name "$SSID" ssid "$SSID" || exit
                    fi
