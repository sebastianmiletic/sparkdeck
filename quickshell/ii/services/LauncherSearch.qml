...
Singleton {
    ...
    Connections {
        ...
        function onAssistantMessageCreated(message) {
            if (root.aiConversationActive)
                root.aiResponseMessage = message;
        }

        function onMessageIDsChanged() {
            if (!root.aiConversationActive || root.aiResponseMessage)
                return;
            const lastId = Ai.messageIDs[Ai.messageIDs.length - 1];
            const msg = Ai.messageByID[lastId];
            if (msg && msg.role === Ai.interfaceRole)
                root.aiResponseMessage = msg;
        }
    }
